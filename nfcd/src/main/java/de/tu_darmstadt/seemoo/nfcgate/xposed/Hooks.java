...
public class Hooks implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(final LoadPackageParam lpparam) {
        ...
        if ("de.tu_darmstadt.seemoo.nfcgate".equals(lpparam.packageName)) {
            ...
            // New hook to force NFCGate as default for payment category
            findAndHookMethod("android.nfc.cardemulation.CardEmulation", lpparam.classLoader,
                    "getDefaultServiceForCategory", String.class, new XC_MethodHook() {
                        @Override
                        protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                            String category = (String) param.args[0];
                            if ("payment".equals(category)) {
                                ComponentName nfcGateService = new ComponentName("de.tu_darmstadt.seemoo.nfcgate", "de.tu_darmstadt.seemoo.nfcgate.nfc.hce.NFCGateHostApduService");
                                param.setResult(nfcGateService);
                                Log.i("HOOKNFC", "Forced NFCGate as default for payment category");
                            }
                        }
                    });
        }
    ...
    private Class<?> findSpecificNfcService(ClassLoader classLoader) {
        List<String> candidates = List.of(
                "com.google.android.nfc.NfcService",
